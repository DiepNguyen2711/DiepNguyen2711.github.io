---
layout: about
title: About Me
permalink: /
subtitle: 

profile:
  align: left
  image: prof_pic.jpg
  image_circular: true 
  address: >
    <div style="text-align: center; margin-top: 15px;">
      <h3 style="margin-bottom: 5px;">Diệp Nguyễn</h3>
      <p style="font-size: 0.9rem;">
        <strong>Nhà nghiên cứu Kinh tế</strong><br>
        Chuyên viên Phân tích Dữ liệu
      </p>
    </div>

news: false
selected_papers: false
social: true
---

Xin chào, tôi là Điệp Nguyễn. 

Tôi là một nhà nghiên cứu kinh tế và phân tích dữ liệu. Chuyên môn của tôi tập trung vào kinh tế vĩ mô, kinh tế lượng, và áp dụng các công cụ lập trình để giải quyết các bài toán kinh tế phức tạp.

<br>

<div class="row">
  <div class="col-sm-6">
    <h4 style="margin-bottom: 15px;">Lĩnh vực quan tâm</h4>
    <ul>
      <li>Kinh tế vĩ mô (Macroeconomics)</li>
      <li>Kinh tế lượng (Econometrics)</li>
      <li>Phân tích dữ liệu bằng Stata & Python</li>
    </ul>
  </div>
  <div class="col-sm-6">
    <h4 style="margin-bottom: 15px;">Học vấn</h4>
    <ul style="list-style-type: none; padding-left: 0;">
      <li>🎓 <strong>Cử nhân Kinh tế Quốc tế</strong><br> <span style="font-size: 0.9em; color: gray;">Đại học Ngoại Thương</span></li>
      <li style="margin-top: 10px;">📖 <strong>Case kinh tế trong thực tế</strong><br> <span style="font-size: 0.9em; color: gray;">Nghiên cứu tự do</span></li>
    </ul>
  </div>
</div>
